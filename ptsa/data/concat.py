...
def concat(
    ...
    coords : {"minimal", "different", "all"} or list of str, optional
        ...
        * ``"different"`` — Coordinates which are not equal (ignoring
          ...
          all for which the dimension already appears). Beware: this option
          may load coordinate variable payloads into memory if not already
          loaded.
        * ``"all"`` — All coordinate variables are concatenated, except
          those corresponding to other dimensions.
        * list of str — The listed coordinate variables are concatenated,
          in addition to the ``"minimal"`` coordinates.
    compat : {"identical", "equals", "broadcast_equals", "no_conflicts", "override"}, optional
        How to compare non-concatenated variables of the same name for
        potential conflicts. Passed down to ``merge``.
        ...
        * ``"override"`` — skip comparing and pick the variable from the
          first dataset.
    ...
    fill_value : scalar or dict-like, optional
        ...
        variable names to fill values. Use a data array's name to refer to
        its values.
    join : {"outer", "inner", "left", "right", "exact"}, optional
        How to combine differing indexes (excluding ``dim``) in objects:
        ...
        * ``"override"`` — if indexes are of same size, rewrite indexes to
          be those of the first object with that dimension. Indexes for the
    ...
    combine_attrs : {"drop", "identical", "no_conflicts", "drop_conflicts", "override"} or callable, optional
        How to combine attrs of the objects being merged (default:
        ``"override"``):

        * ``"drop"`` — empty attrs on returned Dataset.
        * ``"identical"`` — all attrs must be the same on every object.
        * ``"no_conflicts"`` — attrs from all objects are combined; any
          that have the same name must also have the same value.
        * ``"drop_conflicts"`` — attrs from all objects are combined; any
          that have the same name but different values are dropped.
        * ``"override"`` — skip comparing and copy attrs from the first
          dataset to the result.
    ...
    concatenated : TimeSeries
        Concatenation of ``objs`` along ``dim``.
    """

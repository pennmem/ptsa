...
class TestEDFReader:
    ...
    @pytest.mark.parametrize('subject,session', [('LTP360', 2)])
    def test_1_offset_1_chann_returns(self, subject, session):
        ...
        EDFRawReader(dataroot=filename, channels=channel,
                     start_offsets=np.array([0]), read_size=500).read()
    ...
    def test_eeg_reader(self, subject, session):
        ...
        events = BaseEventReader(filename=jr.get_value('task_events',
                                 subject=subject, session=session)).read()

        # hack to make the scalp events work with Rhino mounted
        events['eegfile'] = get_rhino_root() + events[0]['eegfile']

        channels = np.array([0, 1])
        eeg = EEGReader(events=events[:10], channels=channels, start_time=0.0,
                        end_time=0.1).read()
